...
function Login({ handleClose, open, handleOpen1, isAuth }) {
  ...
  const responseSuccessGoogle = (response) => {
    ...
    axios
      .post("https://pinterest-backend-server.herokuapp.com/gauth", payload)
      .then(({ data }) => {
        console.log(data);
        setToken(data.token);
        setGUser(data.user);
        ...
        >
          <Fade in={open}>
            <Box className={styles.outerBox} sx={style}>
              ...
              <button onClick={handleClick} className={styles.modalLogin}>
                <p
                  style={{
                    fontSize: "14px",
                    fontWeight: "bold",
              ...
              <p className={styles.modalLastP1}>
                Are you a business? <span>Get started here!</span>
              </p>
            </Box>
          </Fade>
        </Modal>
